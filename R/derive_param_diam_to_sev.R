...
#' 
#' Helps to filter the diameter records to derive the severity records by passing 
#' the `FATESTCD` value for diameter  which is corresponding to the specified 
#' events in `filter_faobj`.
#'  
...
#' 
#' @param test_sev `FATEST` value foe severity 
#' 
#' *Default: "Severity"*
#' *Permitted Value*: A Character scalar
#' 
...
#' 
#' The `none` and the following arguments(`mild`, `mode` and `sev` ) will be used
#'  for assigning the diameter limits to derive the `AVALC`(severity grade).
#'  
...
#' 
#' @details
#' 1. Pass the input data set in `dataset` with required variables and `AVAL` should
#' have the diameter values for the events(`FAOBJ`) specified in `filter_faobj`.
#' 2. If the SDTM data set has severity record for redness and swelling, User can
#'  skip this function to keep the SDTM level severity records for further 
#'  derivation.
...
#'  will remove the existing severity records and it will create the new severity
#'  records as`AVALC` which is derived from diameter. and `AVAL` will be derived
#'  as numeric severity grade to get the maximum severity records which will be
#'  derived by `derive_param_maxsev.R`.
...
#' 
#' @return 
#' The input data with the new severity records for Redness and swelling which is 
#' specified in `filter_faobj` and AVAL, AVALC will be derived and fatestcd, fatest
...
#'
#' Creating function 
derive_param_diam_to_sev <- function(
  dataset=NULL,
  filter_diam = "DIAMETER",
  ...
  assert_data_frame(dataset,
                    required_vars = vars(USUBJID, AVAL, AVALC, 
                                         FAOBJ, FATEST, FATESTCD))
  
  assert_numeric_vector(arg = c(none,mild,mod,sev),optional = F)
  
  assert_character_vector(arg=c(filter_diam,filter_faobj, testcd_sev,test_sev),
                          optional=F)
  ...
  if(testcd_sev %in% diam$FATESTCD){
    ...
    Input <- dataset %>% filter(FATESTCD != testcd_sev & !(FAOBJ %in% filter_faobj))
  ...
  if(filter_diam %in% diam$FATESTCD){
    ...
    sev <- Input %>% filter(FATESTCD %in% filter_diam & FAOBJ %in% filter_faobj)%>%
      mutate(
        FATESTCD = testcd_sev,
        FATEST = test_sev,
        DTYPE = "DERIVED",
