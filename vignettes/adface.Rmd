...
This call would return the input dataset with columns EXTRT, EXDOSE, EXSTDTC, EXENDTC, VISIT, VISITNUM added if the subjects does not have multiple vaccination at same visit.

If subjects have multiple vaccination at same visit then this call would return the same input dataset with some pre-processing steps added to it.
...
dataset_vignette(
  adface,
  display_vars = exprs(USUBJID, FAOBJ, AVAL, AVALC, ATPTREF, ATPTN, VISIT, VISITNUM)
)
```
...
This call returns the input dataset with severity records added from the diameter records for an event.
...
lookup_dataset <- tribble(
  ...
  "DIAMETER",   "DIARE",     "Diameter",             "Redness",
  "MAXDIAM",    "MDIRE",     "Maximum Diameter cm",  "Redness",
  "MAXTEMP",    "MAXFEV",    "Maximum Temperature",  "Fever",
