...
test_that('derive_vars_merged_vaccine test 1 - Merging EXTRT variable from EX to FACE',{
  ...
  expect_dfs_equal(actual, expected, keys = c('USUBJID','FAOBJ','FATESTCD','FATPTREF',
                                              'FALOC','FALAT'))
})
...
test_that('derive_vars_merged_vaccine test 2 - Check if supp datasets merged
          properly if they exist',{
            face <- tribble(
              ~STUDYID,~DOMAIN,~USUBJID,  ~FACAT, ~FASCAT, ~FATESTCD,  ~FAOBJ, ~FATEST,~FALOC, ~FALAT,~FATPTREF,
              ~FASEQ,
              "ABC","FACE","ABC101","REACTO","ADMINISTRATION SITE", "SEV",     "Redness", "Severity","ARM",
              "RIGHT","VAC 1",1,
              "ABC","FACE","ABC101","REACTO","ADMINISTRATION SITE", "DIAMETER","Redness", "Diameter","ARM",
              "LEFT","VAC 1",2,
              "ABC","FACE","ABC101","REACTO","ADMINISTRATION SITE", "MAXDIAM", "Redness", "Maximum Diameter"
              ,NA,NA,"VAC 2",3,
              "ABC","FACE","ABC101","REACTO","SYSTEMIC", "OCCUR", "Fatigue","Occurrence","LEG","LEFT","VAC 3",5,
              "ABC","FACE","ABC101","REACTO","ADMINISTRATION SITE", "OCCUR", "Erythema","Occurrence","LEG",
              "LEFT","VAC 3",6,
              "ABC","FACE","ABC101","REACTO","ADMINISTRATION SITE", "SEV",   "Swelling","Severity",NA,NA,
              "VAC 4",7,
              "ABC","FACE","ABC101","REACTO","ADMINISTRATION SITE", "OCCUR", "Swelling","Occurrence",NA,NA,
              "VAC 4",8,
              "ABC","FACE","ABC102","REACTO","ADMINISTRATION SITE", "OCCUR", "Swelling", "Occurrence",NA,NA,
              "VAC 1",1
            )
            ...
            suppface <- tribble(
              ...
              "ABC","ABC101","FACE","FASEQ",1,"CLTYP","DAIRY","Collection Type","Predecessor",
              "ABC","ABC101","FACE","FASEQ",2,"CLTYP","CRF","Collection Type","Predecessor"
            )

            temp <- suppface %>%   pivot_wider(id_cols = c(USUBJID,IDVAR,IDVARVAL),
                                               names_from = QNAM,
                                               values_from = QVAL) %>%
            ...
            expected1<- left_join(admin,ex1,by = c("USUBJID","LOC","LAT","TPTREF"), keep = FALSE) %>%
              select(-c("LOC","LAT","TPTREF"))
...
test_that('derive_vars_merged_vaccine test 1 - Checking if scenario handled for
          FADIR,FALOC, FALAT are missing',{
            face <- tribble(
              ...
              "ABC101","REACTO","ADMINISTRATION SITE", "DIAMETER","Redness", "Diameter","VAC 1",
              "ABC101","REACTO","ADMINISTRATION SITE", "MAXDIAM", "Redness", "Maximum Diameter",
              "VAC 2",
              "ABC101","REACTO","SYSTEMIC", "OCCUR", "Fatigue","Occurrence","VAC 3",
              "ABC101","REACTO","ADMINISTRATION SITE", "OCCUR", "Erythema","Occurrence","VAC 3",
              "ABC101","REACTO","ADMINISTRATION SITE", "SEV",   "Swelling","Severity","VAC 4",
              "ABC101","REACTO","ADMINISTRATION SITE", "OCCUR", "Swelling","Occurrence","VAC 4",
              "ABC102","REACTO","ADMINISTRATION SITE", "OCCUR", "Swelling", "Occurrence","VAC 1"
            )
