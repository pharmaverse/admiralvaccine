...
table1$USUBJID <- c(rep("ABC-1001",8),rep("ABC-1002",8),rep("ABC-1003",8),rep("ABC-1004",8),
...
is1 <- table1 %>%
  ...
  mutate(EPOCH = if_else(VISITNUM == 10, "FIRST TREATMENT","SECOND TREATMENT"),
         ...
         ULLOQ = case_when(ISTESTCD == "J0033VN" ~ 100,
...
is <- is1 %>%
  mutate(ISORRES = if_else(row_number() == 1 | row_number() == 10, as.character(NA), ISORRES),
         ...
         # Add missing dates
         ISDTC = case_when(row_number() == 1 ~ "1999-01",
         ...
         ISORRES = case_when(row_number() == 8 ~ "140.5",
                             row_number() == 15 ~ "98.2",
                             row_number() == 24 ~ "48.9",
                             row_number() == 29 ~ "228.1",
         ...
         ISSTRESC = case_when(row_number() == 8 ~ "140.5",
                              row_number() == 15 ~ "98.2",
                              row_number() == 24 ~ "48.9",
                              row_number() == 29 ~ "228.1",
         ...
         ISSTRESN = case_when(row_number() == 8 ~ 140.5,
                              row_number() == 15 ~ 98.2,
                              row_number() == 24 ~ 48.9,
                              row_number() == 29 ~ 228.1,
         ...
         ISORRES = case_when(ISORRES == ">8" & ISTESTCD == "J0033VN" ~ ">100",
