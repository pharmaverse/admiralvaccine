...
vs <- vs_dummy %>% mutate(VSSEQ = 1:14,
                          USUBJID = "ABC-1001") %>%
  bind_rows(vs_dummy %>% mutate(VSSEQ = 1:14,
                                USUBJID = "ABC-1002")) %>%
  mutate(
    STUDYID = "ABC",
    DOMAIN = "VS",
...
vs3 <- bind_cols(vs,vs2) %>% mutate(VSEVAL = ifelse(!is.na(VSSTRESC),"STUDY SUBJECT",NA_character_))

# ordering the varibales
vx_vs <- vs3 %>% select(STUDYID,DOMAIN,USUBJID,VSSEQ,VSLNKID,VSLNKGRP,VSTESTCD,VSTEST,VSCAT,
                     VSSCAT,VSORRES,VSORRESU,VSSTRESC,VSSTRESN,VSSTRESU,VSEVAL,VSLOC,
                     EPOCH,VSDTC,VSDY,VSTPT,VSTPTNUM,VSTPTREF) %>% ungroup()

dir <- tempdir()
save(vx_vs, file = file.path(dir, "vx_vs.rda"), compress = "bzip2")
