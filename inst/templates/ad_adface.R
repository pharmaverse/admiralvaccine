...
check <- adface %>% group_by(USUBJID,FAOBJ,FATESTCD) %>%
  count(FATESTCD)
...
check1 <- adface %>% group_by(USUBJID,FAOBJ,FATESTCD) %>%
  count(FATESTCD)
...
check2 <- adface %>% group_by(USUBJID,FAOBJ,FATESTCD) %>%
  count(FATESTCD)
...
check3 <- adface %>% group_by(USUBJID,FAOBJ,FATESTCD) %>%
  count(FATESTCD)
...
period_ref <- create_period_dataset(dataset = adsl,
